...
点击“词汇定级”可以完成 18 道自适应词义题。测试从 B1 难度开始，当前依次覆盖 A1、A2、B1、B2 和 C1；答对后提高下一题难度，答错或选择“不知道”后降低难度。能力值使用逐题更新的 logistic 概率模型，不用固定正确率直接套级别。定级题与普通学习词分开筛选：A1-B2 使用 Oxford 核心词，C1 使用 Oxford 与 Octanove C1 核心词。C2/Beyond C1 数据暂时保留但停用，CEFR-J 单独补充的主题词和 `Unrated` 词也不会进入测试题或干扰选项。
...
完成测试后，新词约 75% 来自测得级别，25% 来自高一级。例如 B1 学习范围为 B1 和 B2；C1 是当前最高档，因此 C1 结果只抽取 C1 新词。C2/Beyond C1 新词暂不抽取，此前已经学过的词仍会按记录的词序位置复习。
...
- “备份进度”导出 schema v4 JSON，其中包含完整词库、逐卡词距调度状态、全局学习位置、设置、每日统计、定级结果和未完成测试。
